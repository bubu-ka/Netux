/* 单支短线策略 component
 * 数据: frontend/data/short_range.json  由 backend/components/short_range/ 生产
 *
 * 视图:
 *   - 唯一单支短线策略说明 + 明日操作统计
 *   - 榜单:明日建议、规则评分、策略/股票近 2 周和近 1 月表现、近期信号
 *   - 点击行 → 最近 1 个月日 K + 买卖点 + 策略交易 + 操作依据
 */
(function () {
  "use strict";

  const FAVORITES_KEY = "nexus_favorites";
  const DATA_URL = "data/short_range.json";
  ...
  function renderStrategyHeader(s) {
    const strategyDescriptions = [
      {
        name: "short_swing",
        title: "EMA-RSI-ATR短线",
        text: "关注最近两周的短期趋势启动：EMA5/EMA20判断方向，RSI过滤过热追高，ATR控制止损、止盈和移动止损。",
      },
    ];
    ...
    return `
      <div class="range-strategy-card range-explain">
        <div class="rs-head">
          <div class="rs-title">
            <span class="rs-name">${s.title || "智能匹配单支短线策略"}</span>
        ...
        <div class="rs-desc">${esc(s.desc || "不同标的波动结构不同；分类器会结合趋势结构、近期表现、超额收益、回撤控制和明日信号，为每个标的选择更合适的单支短线策略。")}</div>
  ...
  // ---------- render: table ----------
  const SORT_COLUMNS = {
    strategy_1m: (it) => metric(it, "strategy_return_2w_pct", "return_2w_pct"),
    stock_1m: (it) => metric(it, "stock_return_2w_pct", "bnh_2w_pct"),
    strategy_1y: (it) => metric(it, "strategy_return_1m_pct", "return_1m_pct"),
    stock_1y: (it) => metric(it, "stock_return_1m_pct", "bnh_1m_pct"),
    excess_1y: (it) => metric(it, "excess_2w_pct", "excess_2w_pct"),
      ...
      <div class="panel range-table-panel">
        <div class="panel-head">
          <div class="panel-title">
            短线策略榜单
            <small>当前 ${assetTypeLabel(assetFilter)} · 共 ${total} 个匹配标的 · 每页 ${PAGE_SIZE} 个 · 点击查看最近1个月日K、买卖点和近期信号</small>
          </div>
        </div>
          ...
          <table class="range-table">
            <thead>
              <tr>
                ...
                <th>适用策略</th>
                <th>近期提示</th>
                ${sortTh("strategy_1m", "策略近2周")}
                ${sortTh("stock_1m", "股票近2周")}
                ${sortTh("strategy_1y", "策略近1月")}
                ${sortTh("stock_1y", "股票近1月")}
                ${sortTh("excess_1y", "超额2周")}
                ${sortTh("capture", "捕获率")}
                <th>阶段</th>
                <th>当前状态</th>
                <th>最近信号</th>
  ...
  function rowOf(it, strategy) {
    ...
    return `
      <tr class="range-row ${activeCls}" data-sym="${it.symbol}">
        ...
        <td class="num ${pctClass(metric(it, "strategy_return_2w_pct", "return_2w_pct"))} strong">${fmtPct(metric(it, "strategy_return_2w_pct", "return_2w_pct"))}</td>
        <td class="num ${pctClass(metric(it, "stock_return_2w_pct", "bnh_2w_pct"))}">${fmtPct(metric(it, "stock_return_2w_pct", "bnh_2w_pct"))}</td>
        <td class="num ${pctClass(metric(it, "strategy_return_1m_pct", "return_1m_pct"))}">${fmtPct(metric(it, "strategy_return_1m_pct", "return_1m_pct"))}</td>
        <td class="num ${pctClass(metric(it, "stock_return_1m_pct", "bnh_1m_pct"))}">${fmtPct(metric(it, "stock_return_1m_pct", "bnh_1m_pct"))}</td>
        <td class="num ${pctClass(metric(it, "excess_2w_pct", "excess_2w_pct"))} strong">${fmtPct(metric(it, "excess_2w_pct", "excess_2w_pct"))}</td>
  ...
  function renderDetailMeta(item) {
    ...
    return `
      <div class="detail-meta">
        ${posPart}
        <span class="dm-item ${pctClass(metric(item, "strategy_return_2w_pct", "return_2w_pct"))}">
          策略近2周 <b>${fmtPct(metric(item, "strategy_return_2w_pct", "return_2w_pct"))}</b>
        </span>
        ...
        <span class="dm-item ${pctClass(metric(item, "stock_return_1m_pct", "bnh_1m_pct"))}">
          股票近1月 <b>${fmtPct(metric(item, "stock_return_1m_pct", "bnh_1m_pct"))}</b>
        </span>
        <span class="dm-item ${pctClass(metric(item, "excess_2w_pct", "excess_2w_pct"))}">
          超额 <b>${fmtPct(metric(item, "excess_2w_pct", "excess_2w_pct"))}</b>
        </span>
  ...
  function renderRecentSignals(item) {
    ...
    if (!sigs.length) {
      return `<div class="recent-sig-empty">近 2 周内该策略无买卖信号</div>`;
    }
  ...
  function renderDetailShell(s, summaryItem, detailItem) {
    if (!summaryItem) {
      return `<div class="panel range-detail-panel"><div class="state">点击榜单行查看最近1个月日K、买卖点和交易明细</div></div>`;
    }
  ...
  function sortStocks(stocks) {
    ...
    return stocks.slice().sort((a, b) => {
      ...
      return (metric(b, "strategy_return_2w_pct", "return_2w_pct") ?? -1e9)
        - (metric(a, "strategy_return_2w_pct", "return_2w_pct") ?? -1e9);
    });
  }
    ...
    try {
      ...
      host.innerHTML = `
        <div class="state error">
          ...
          请先生成静态数据:<code>bash scripts/generate.sh short_range</code>
        </div>`;
      return;
    }
    const s = getStrategy();
  ...
  window.NexusComponents["short_range"] = {
    id: "short_range",
    title: "单支短线策略",
    group: "技术分析",
    render,
